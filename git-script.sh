...
# Get the github access token from a local file
source ~/scripts/access-token.txt
...
curl -H "Authorization: token $token" https://api.github.com/user/repos -d "{\"name\": \"${REPO_NAME}\", \"description\": \"${DESCRIPTION}\"}"
